...
// ===== DESIGN TOKENS: Mandinga Design System =====
// Updated to support proper contrast, accessibility, and visual hierarchy

// Color Palette - Mandinga Accent System
const COLORS = {
  // Mandinga Indigo/Purple (primary accent for dots, progress, highlights)
  primary: "#6366F1",      // Indigo-500
  primaryLight: "#C7D2FE", // Indigo-200 (early entry dots)
  ...
  textPrimary: "#1F2937",  // slate-900 (main text)
  textSecondary: "#6B7280", // slate-500 (labels, secondary)
  textMuted: "#9CA3AF",     // slate-400 (captions)
...
const TYPOGRAPHY = {
  ...
  display: `text-5xl font-bold text-${COLORS.textPrimary}`,
  
  // Headings: Card titles, section headers
  h1: `text-2xl font-bold text-[${COLORS.textPrimary}]`,
  h2: `text-lg font-semibold text-[${COLORS.textPrimary}]`, 
  h3: `text-base font-semibold text-[${COLORS.textPrimary}]`,
  
  // Body text: Default paragraph content
  body: `text-sm font-normal text-[${COLORS.textPrimary}]`,
  
  // Body muted: Secondary copy, descriptions (INCREASED to text-sm for readability)
  bodyMuted: `text-sm text-[${COLORS.textSecondary}]`,
  
  // Labels: Small labels like "Started on", "Payout" (4.5:1 contrast)
  label: `text-sm text-[${COLORS.textSecondary}]`,
  
  // Caption: Tiny metadata like "1d ago", "01/24"
  caption: `text-xs text-[${COLORS.textMuted}]`,
  
  // Button: Button and link text
  button: `text-sm font-medium text-[${COLORS.primary}]`,
}
...
const PADDING_L = "p-6"    // 24px (updated to p-6 per master prompt)
const PADDING_XL = "p-8"   // 32px
...
// Grid gap token (24px - the "golden gap")
...
// Compact, content-driven layout with high-contrast labels
function TimelineCard() {
  return (
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
      {/* Row 1: Started on */}
      <div className="flex items-center justify-between">
        <p className={`text-sm text-[${COLORS.textSecondary}]`}>Started on</p>
        <p className={`font-semibold text-[${COLORS.textPrimary}]`}>{circleData.startDate}</p>
      </div>
      {/* Row 2: Ends on */}
      <div className="flex items-center justify-between">
        <p className={`text-sm text-[${COLORS.textSecondary}]`}>Ends on</p>
        <p className={`font-semibold text-[${COLORS.textPrimary}]`}>{circleData.endDate}</p>
      </div>
    </div>
  )
}
...
// Header: Title | Counter ("01/24") with Mandinga purple progress bar
...
function PayoutCard({ isWalletConnected, hasJoined }: { isWalletConnected: boolean; hasJoined: boolean }) {
  ...
  if (isPreJoin) {
    return (
      <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
        ...
        <div className="flex items-center justify-between">
          <span className={`text-sm text-[${COLORS.textSecondary}]`}>Payout</span>
          <span></span>
        </div>

        {/* Progress bar with 1% initial fill (Mandinga purple) */}
        <div className={`h-2 w-full overflow-hidden rounded-full bg-[${COLORS.border}]`}>
          <div className={`h-full w-[1%] bg-[${COLORS.primary}] rounded-full`} />
        </div>

        {/* Content row: text left, amount right */}
        <div className="flex items-center justify-between">
          <span className={`font-medium text-[${COLORS.textPrimary}]`}>Always due on the 5th, every month</span>
          <span className={`font-semibold text-[${COLORS.textPrimary}]`}>${formatNumber(circleData.amount)}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
      ...
      <div className="flex items-center justify-between">
        <span className={`text-sm text-[${COLORS.textSecondary}]`}>Payout</span>
        <span className={`text-xs text-[${COLORS.textMuted}]`}>
          {String(circleData.payoutProgress).padStart(2, "0")}/{circleData.totalMonths}
        </span>
      </div>

      {/* Progress bar with fill indicator (Mandinga purple) */}
      <div className={`h-2 w-full overflow-hidden rounded-full bg-[${COLORS.border}]`}>
        <div
          className={`h-full bg-[${COLORS.primary}] rounded-full transition-all`}
      ...
      {/* Content row: text left, amount right */}
      <div className="flex items-center justify-between">
        <span className={`font-medium text-[${COLORS.textPrimary}]`}>Always due on the 1st, every month</span>
        <span className={`font-semibold text-[${COLORS.textPrimary}]`}>${formatNumber(circleData.amount)}</span>
      </div>
    </div>
  )
    ...
    <div ref={containerRef} className="w-full">
      ...
      >
        {dots.map((i) => {
          ...
          if (isFilled) bgColor = COLORS.primary
          else if (isEarlyEntry) bgColor = COLORS.primaryLight
...
// Content-driven layout: Title → Description → Dots Grid (no h-full, no justify-between)
// Layout uses flex-col gap-4 as per master prompt
function PaymentVisualizationCard() {
  return (
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M} w-full`}>
      <div>
        <h2 className={`text-base font-semibold text-[${COLORS.textPrimary}]`}>
          Pay ${formatNumber(circleData.monthlyAmount)} /mo for {circleData.totalMonths} months
        </h2>
        <p className={`text-sm text-[${COLORS.textSecondary}] mt-1`}>
...
function InstallmentCard({ isWalletConnected, hasJoined }: { isWalletConnected: boolean; hasJoined: boolean }) {
  ...
  if (isPreJoin) {
    return (
      <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
        {/* Header: Title | Counter (empty pre-join) */}
        <div className="flex items-center justify-between">
          <span className={`text-sm text-[${COLORS.textSecondary}]`}>Installments</span>
          <span></span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className={`font-medium text-[${COLORS.textPrimary}]`}>Always due on the 5th, every month</span>
          <span className={`font-semibold text-[${COLORS.textPrimary}]`}>${formatNumber(circleData.monthlyAmount)}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
      ...
      <div className="flex items-center justify-between">
        <span className={`font-medium text-[${COLORS.textPrimary}]`}>Always due on the 5th, every month</span>
        <span className={`font-semibold text-[${COLORS.textPrimary}]`}>${formatNumber(circleData.dueAmount)}</span>
      </div>
    </div>
  )
}
...
// Header (two-column: logo+name | link) with high-contrast text
function EnsCard() {
  return (
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-base font-semibold text-[#5298FF]`}>ens</span>
        </div>
        <a
          href={circleData.ensUrl}
          target="_blank"
          rel="noopener noreferrer"
          className={`text-sm font-medium text-[${COLORS.primary}] transition-colors hover:opacity-70 whitespace-nowrap`}
        >
          View on ENS →
        </a>
      </div>

      {/* Description - muted, below header (increased to text-sm) */}
      <p className={`text-sm text-[${COLORS.textSecondary}]`}>
        Public ENS name for this circle's vault.
      </p>

      {/* Primary Object - ENS pill */}
      <div className={`rounded-full bg-[${COLORS.primaryLight}] px-4 py-2 text-center`}>
        <span className={`text-sm font-medium text-[${COLORS.primary}]`}>{circleData.ensDomain}</span>
      </div>
    </div>
  )
}
...
// Compact member list with high-contrast typography
function MembersCard() {
  ...
  return (
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
      <h3 className={`text-base font-semibold text-[${COLORS.textPrimary}]`}>Active members</h3>
      ...
      <div>
        {circleData.members.slice(0, maxVisibleMembers).map((member, index) => (
          <div 
            key={member.name} 
            className={`flex items-center justify-between gap-3 py-2 min-w-0 ${index < Math.min(circleData.members.length, maxVisibleMembers) - 1 ? `border-b border-[${COLORS.border}]` : ''}`}
          >
            <span className={`text-sm text-[${COLORS.textPrimary}] truncate min-w-0`}>{member.name}</span>
            <span className={`text-xs text-[${COLORS.textMuted}] whitespace-nowrap flex-shrink-0`}>{formatJoinDate(member.joinedDaysAgo)}</span>
          </div>
        ))}
      </div>
      ...
      {hasMoreMembers && (
        <p className={`text-xs text-[${COLORS.textMuted}]`}>
    ...
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex flex-col ${GAP_M}`}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className={`text-base font-semibold text-[${COLORS.textPrimary}]`}>Arc</span>
        </div>
        <a
          href={circleData.arcscanUrl}
          target="_blank"
          rel="noopener noreferrer"
          className={`text-sm font-medium text-[${COLORS.textPrimary}] transition-colors hover:opacity-70 whitespace-nowrap`}
        >
          View on Arcscan →
        </a>
      </div>
...
// Content-driven, flex layout with space-between (not h-full)
function SlotsCard() {
  return (
    <div className={`rounded-xl border ${`border-[${COLORS.border}]`} bg-white ${PADDING_L} flex items-center justify-between`}>
      {/* Active badge */}
      <div className={`flex items-center gap-2 rounded-2xl bg-[${COLORS.successLight}] px-3 py-1.5`}>
        <span className={`h-2 w-2 rounded-full bg-[${COLORS.success}]`} />
        <span className={`text-sm font-medium text-[${COLORS.success}]`}>Active</span>
      </div>
      {/* Slots count */}
      <span className={`text-sm font-medium text-[${COLORS.textSecondary}]`}>{circleData.slotsLeft} slots left</span>
    </div>
  )
}
...
export default function FundingCirclePage() {
  ...
  return (
    <div className="min-h-screen bg-white flex flex-col">
      ...
      <main className="flex-1 flex flex-col justify-center mx-auto max-w-7xl w-full px-6 md:px-10 pb-12 pt-4 box-border">
