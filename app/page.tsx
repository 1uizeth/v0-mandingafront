...
// Typography System - Static Tailwind classes only
const TYPOGRAPHY = {
...
function TimelineCard() {
  return (
    <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
      {/* Row 1: Started on */}
      <div className="flex items-center justify-between">
        <p className={`${TYPOGRAPHY.label}`} style={{ color: COLORS.textSecondary }}>Started on</p>
        <p className="font-semibold" style={{ color: COLORS.textPrimary }}>{circleData.startDate}</p>
      </div>
      {/* Row 2: Ends on */}
      <div className="flex items-center justify-between">
        <p className={`${TYPOGRAPHY.label}`} style={{ color: COLORS.textSecondary }}>Ends on</p>
        <p className="font-semibold" style={{ color: COLORS.textPrimary }}>{circleData.endDate}</p>
      </div>
    </div>
  )
}
...
function PayoutCard({ isWalletConnected, hasJoined }: { isWalletConnected: boolean; hasJoined: boolean }) {
  ...
  if (isPreJoin) {
    return (
      <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
        ...
        <div className="flex items-center justify-between">
          <span className={TYPOGRAPHY.label} style={{ color: COLORS.textSecondary }}>Payout</span>
          <span></span>
        </div>

        {/* Progress bar with 1% initial fill (Mandinga purple) */}
        <div className="h-2 w-full overflow-hidden rounded-full" style={{ backgroundColor: COLORS.border }}>
          <div className="h-full w-[1%] rounded-full" style={{ backgroundColor: COLORS.primary }} />
        </div>

        {/* Content row: text left, amount right */}
        <div className="flex items-center justify-between">
          <span className="font-medium" style={{ color: COLORS.textPrimary }}>Always due on the 5th, every month</span>
          <span className="font-semibold" style={{ color: COLORS.textPrimary }}>${formatNumber(circleData.amount)}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
      ...
      <div className="flex items-center justify-between">
        <span className={TYPOGRAPHY.label} style={{ color: COLORS.textSecondary }}>Payout</span>
        <span className={TYPOGRAPHY.caption} style={{ color: COLORS.textMuted }}>
      ...
      {/* Content row: text left, amount right */}
      <div className="flex items-center justify-between">
        <span className="font-medium" style={{ color: COLORS.textPrimary }}>Always due on the 1st, every month</span>
        <span className="font-semibold" style={{ color: COLORS.textPrimary }}>${formatNumber(circleData.amount)}</span>
      </div>
    </div>
  )
...
// Content-driven layout: Title → Description → Dots Grid
function PaymentVisualizationCard() {
  return (
    <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M} w-full`} style={{ borderColor: COLORS.border }}>
      <div>
        ...
        <p className={`${TYPOGRAPHY.bodyMuted} mt-1`} style={{ color: COLORS.textSecondary }}>
...
function InstallmentCard({ isWalletConnected, hasJoined }: { isWalletConnected: boolean; hasJoined: boolean }) {
  ...
  if (isPreJoin) {
    return (
      <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
        {/* Header: Title | Counter (empty pre-join) */}
        <div className="flex items-center justify-between">
          <span className={TYPOGRAPHY.label} style={{ color: COLORS.textSecondary }}>Installments</span>
          <span></span>
        </div>
        ...
        <div className="flex items-center justify-between">
          <span className="font-medium" style={{ color: COLORS.textPrimary }}>Always due on the 5th, every month</span>
          <span className="font-semibold" style={{ color: COLORS.textPrimary }}>${formatNumber(circleData.monthlyAmount)}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
      ...
      <div className="flex items-center justify-between">
        <span className="font-medium" style={{ color: COLORS.textPrimary }}>Always due on the 5th, every month</span>
        <span className="font-semibold" style={{ color: COLORS.textPrimary }}>${formatNumber(circleData.dueAmount)}</span>
      </div>
    </div>
  )
}
    ...
    <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-semibold" style={{ color: "#5298FF" }}>ens</span>
        </div>
        <a href={circleData.ensUrl} target="_blank" rel="noopener noreferrer" className={TYPOGRAPHY.button} style={{ color: COLORS.primary }}>
          View on ENS →
        </a>
      </div>
      <p className={TYPOGRAPHY.bodyMuted} style={{ color: COLORS.textSecondary }}>
        Public ENS name for this circle's vault.
      </p>
      <div className="rounded-full px-4 py-2 text-center" style={{ backgroundColor: COLORS.primaryLight }}>
        <span className={TYPOGRAPHY.button} style={{ color: COLORS.primary }}>{circleData.ensDomain}</span>
      </div>
    </div>
  )
}
...
function MembersCard() {
  ...
  return (
    <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
      <h3 className={TYPOGRAPHY.h3} style={{ color: COLORS.textPrimary }}>Active members</h3>
      <div>
        {circleData.members.slice(0, maxVisibleMembers).map((member, index) => (
          <div 
            ...
            style={{ borderBottom: index < Math.min(circleData.members.length, maxVisibleMembers) - 1 ? `1px solid ${COLORS.border}` : 'none' }}
          >
            <span className="text-sm truncate min-w-0" style={{ color: COLORS.textPrimary }}>{member.name}</span>
            <span className={TYPOGRAPHY.caption} style={{ color: COLORS.textMuted }}>{formatJoinDate(member.joinedDaysAgo)}</span>
          </div>
        ))}
      </div>
      {hasMoreMembers && (
        <p className={TYPOGRAPHY.caption} style={{ color: COLORS.textMuted }}>
    ...
    <div className={`rounded-xl border bg-white ${PADDING_L} flex flex-col ${GAP_M}`} style={{ borderColor: COLORS.border }}>
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-2">
          ...
          <span className="text-base font-semibold" style={{ color: COLORS.textPrimary }}>Arc</span>
        </div>
        <a href={circleData.arcscanUrl} target="_blank" rel="noopener noreferrer" className={TYPOGRAPHY.button} style={{ color: COLORS.textPrimary }}>
          View on Arcscan →
        </a>
      </div>
    </div>
...
function SlotsCard() {
  return (
    <div className={`rounded-xl border bg-white ${PADDING_L} flex items-center justify-between`} style={{ borderColor: COLORS.border }}>
      <div className="flex items-center gap-2 rounded-2xl px-3 py-1.5" style={{ backgroundColor: COLORS.successLight }}>
        <span className="h-2 w-2 rounded-full" style={{ backgroundColor: COLORS.success }} />
        <span className={TYPOGRAPHY.button} style={{ color: COLORS.success }}>Active</span>
      </div>
      <span className={TYPOGRAPHY.button} style={{ color: COLORS.textSecondary }}>{circleData.slotsLeft} slots left</span>
    </div>
  )
}
