...
// Design tokens for unified spacing and padding
const PADDING = "p-5" // M token
const SPACE_BETWEEN_SECTIONS = "gap-4" // Between major sections in cards
const SPACE_BETWEEN_ROWS = "gap-2" // Between content rows

// FULL CARD: Timeline
// Two-line layout with "Started on / Ends on" header style (label on left, date on right)
function TimelineCard() {
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} h-full flex flex-col justify-start`}>
      {/* Started on section */}
      <div className={`flex items-center justify-between ${SPACE_BETWEEN_ROWS}`}>
        <p className="text-sm text-[#666666]">Started on</p>
        <p className="text-sm font-semibold text-[#1A1A1A] whitespace-nowrap">{circleData.startDate}</p>
      </div>
      {/* Ends on section */}
      <div className={`flex items-center justify-between ${SPACE_BETWEEN_ROWS}`}>
        <p className="text-sm text-[#666666]">Ends on</p>
        <p className="text-sm font-semibold text-[#1A1A1A] whitespace-nowrap">{circleData.endDate}</p>
      </div>
    </div>
  )
}
...
// Two-column header: Title | Counter
// Progress bar with 1% initial fill indicator
function PayoutCard({ isWalletConnected, hasJoined }: { isWalletConnected: boolean; hasJoined: boolean }) {
  ...
  if (isPreJoin) {
    return (
      <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS}`}>
        ...
        {/* Progress bar with 1% fill */}
        <div className="h-2 w-full overflow-hidden rounded-full bg-[#E5E5E5]">
          <div className="h-full w-[1%] rounded-full bg-[#1A1A1A]" />
        </div>

        {/* Content row: text left, amount right */}
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-[#1A1A1A]">Always due on the 5th, every month</span>
          <span className="text-sm font-semibold text-[#1A1A1A] whitespace-nowrap">${formatNumber(circleData.amount)}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS}`}>
      ...
      {/* Content row: text left, amount right */}
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-[#1A1A1A]">Always due on the 1st, every month</span>
        <span className="text-sm font-semibold text-[#1A1A1A] whitespace-nowrap">${formatNumber(circleData.amount)}</span>
      </div>
    </div>
  )
...
function PaymentVisualizationCard() {
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS} w-full h-full justify-between`}>
      <div>
...
function InstallmentCard({ isWalletConnected, hasJoined }: { isWalletConnected: boolean; hasJoined: boolean }) {
  ...
  if (isPreJoin) {
    return (
      <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS}`}>
        ...
        <div className="flex items-center justify-between">
          <span className="text-sm font-semibold text-[#1A1A1A]">Always due on the 5th, every month</span>
          <span className="text-sm font-semibold text-[#1A1A1A] whitespace-nowrap">${formatNumber(circleData.monthlyAmount)}</span>
        </div>
      </div>
    )
  }
  ...
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS}`}>
      ...
      <div className="flex items-center justify-between">
        <span className="text-sm font-semibold text-[#1A1A1A]">Always due on the 5th, every month</span>
        <span className="text-sm font-semibold text-[#1A1A1A] whitespace-nowrap">${formatNumber(circleData.dueAmount)}</span>
      </div>
    </div>
  )
}
...
// Pattern: Header (two-column: logo+name | link) → Description → ENS pill
function EnsCard() {
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS}`}>
...
function MembersCard() {
  ...
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS}`}>
      ...
      {/* Member list - same text-xs sizing as other cards */}
      <div>
        {circleData.members.slice(0, maxVisibleMembers).map((member, index) => (
          <div 
            key={member.name} 
            className={`flex items-center justify-between gap-3 py-1 min-w-0 ${index < Math.min(circleData.members.length, maxVisibleMembers) - 1 ? 'border-b border-[#F5F5F5]' : ''}`}
          >
...
// Simplified: Header (two-column: logo+name | link) with unified spacing
function ArcCard() {
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white ${PADDING} flex flex-col ${SPACE_BETWEEN_SECTIONS}`}>
...
// Slots card - includes Active badge + slots count with unified spacing
function SlotsCard() {
  return (
    <div className={`rounded-xl border border-[#E5E5E5] bg-white px-5 py-4 flex items-center justify-between gap-4`}>
      {/* Active badge */}
      <div className="flex items-center gap-2 rounded-2xl bg-[#E8F5E9] px-3 py-1">
